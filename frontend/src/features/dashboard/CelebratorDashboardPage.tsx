  ...
  return (
    <div>
      ...
      <motion.div
        ...
        className="relative overflow-hidden rounded-3xl mb-8 p-8 bg-gradient-to-br from-accent-500 via-accent-400 to-amber-300 text-white"
      >
        {/* decorative blobs — CSS only, no external images */}
        ...
        <div className="absolute top-6 right-10 text-5xl opacity-90 rotate-6">🎉</div>
        <div className="absolute bottom-6 right-32 text-3xl opacity-70 -rotate-12">🎈</div>
        <div className="absolute top-16 right-40 text-2xl opacity-60 rotate-12">✨</div>

        <p className="text-xs uppercase tracking-widest text-white/80 font-medium relative">
          Celebrator Dashboard
        </p>
        <h1 className="font-display text-3xl sm:text-4xl mt-1 relative">
          Hi {user?.username}, let's plan something wonderful
        </h1>
        <p className="text-sm text-white/85 mt-2 relative">
          ...
          <div className="grid grid-cols-2 gap-5">
            ...
            <WidgetCard
              ...
              action={savedPlanners.length > 0 && (
                <Link to="/marketplace" className="text-xs text-accent-600 underline">
                  Browse more
                </Link>
              )}
            >
                ...
                <div className="space-y-2">
                  {savedPlanners.slice(0, 4).map((s) => (
                    <Link
                      ...
                      className="flex items-center justify-between text-xs hover:opacity-70"
                    >
                      <span className="text-ink-700 dark:text-ink-200 font-medium truncate">
                        {s.planner.business_name}
                      </span>
                      <span className="text-ink-400 shrink-0 ml-2">{s.planner.city}</span>
                    </Link>
                  ))}
                </div>
              )}
